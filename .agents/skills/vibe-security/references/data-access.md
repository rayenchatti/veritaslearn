...
- **Validate input types with Zod before passing to Prisma.** `findFirst` and similar methods are vulnerable to operator injection if unvalidated objects are passed as filter values.
...
Don't rely on TypeScript types alone — they're compile-time only and don't exist at runtime.
...
Don't spread request bodies directly into database operations:
